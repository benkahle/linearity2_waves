...
function newH=Wave(n,theta,rho,dt,c,k,H,oldH,fix,cont,connect)
    ...
    potential(theta,rho)= -c^2*((4*H(theta,rho)-H(theta+1,rho)-H(theta-1,rho)-H(theta,rho+1)-H(theta,rho-1))...
...
if(~fix && ~cont && ~connect)
    ...
    oldH=H;
    newH=H;
    theta = 2:n-1; %Not the values, but indices 
    rho = 2:n-1; %Not that values, but the indices
